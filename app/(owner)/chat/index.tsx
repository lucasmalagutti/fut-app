    ...
    <SafeAreaView style={styles.safe}>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Conversas com jogadores aparecerão aqui."
        />
      ) : (
        <FlatList
          data={threads}
          ...
          renderItem={({ item }) => {
            ...
            return (
              <Card
                onPress={() =>
                  router.push({ pathname: '/(owner)/chat/[threadId]', params: { threadId: item.id } })
                }
                ...
                <View style={styles.threadRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    <View style={styles.threadHeader}>
                      <Text style={styles.threadName}>{other?.name ?? 'Jogador'}</Text>
                      {item.lastMessageAt && (
                        <Text style={styles.threadTime}>{formatRelativeTime(item.lastMessageAt)}</Text>
                      )}
                    </View>
                    {item.lastMessage && (
                      <Text style={styles.lastMessage} numberOfLines={1}>{item.lastMessage.body}</Text>
                    )}
                  </View>
                </View>
              </Card>
