...
const SPORTS = ['Todos', 'Futebol', 'Society', 'Futsal', 'Basquete', 'Tênis'];
...
export default function PlayerHomeScreen() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['courts', { q: search, sport: selectedSport, lat: location?.latitude, lng: location?.longitude }],
    queryFn: () =>
      courtsService.list({
        q: search || undefined,
    ...
    <SafeAreaView style={styles.safe}>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          onAction={() => { setSearch(''); setSelectedSport('Todos'); }}
        />
      ) : (
        <FlatList
          data={courts}
          ...
          renderItem={({ item }) => (
            <CourtCard
              court={item}
              onPress={() => router.push({ pathname: '/(player)/courts/[id]', params: { id: item.id } })}
            />
          )}
